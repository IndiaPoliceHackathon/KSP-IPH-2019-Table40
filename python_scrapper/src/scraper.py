...
from download_post_images import download_post_images
...
def get_status(x):
    ...
    try:
        status = x.find_element_by_xpath(
            ".//div[@class='_5wj-']").text  # use _1xnd for Pages
    except Exception:
        try:
            status = x.find_element_by_xpath(
...
def get_comments(x):
    comments = x.find_elements_by_xpath("//div [@data-testid='UFI2Comment/body']")
    comments_data=[]

    for comment in comments:
        facebook_id = comment.find_elements_by_class_name("_6qw4")[0].get_attribute("href").split('/')[-1]
        full_name = comment.find_elements_by_class_name("_6qw4")[0].get_attribute("innerHTML")
        comment_text = comment.find_elements_by_class_name("_3l3x")[0].text
        
        comments_data.append(facebook_id+','+full_name+','+comment_text)
...
def extract_and_write_posts(elements, filename):
    try:
        ...
        f.writelines(
            ' TIME || TYPE  || TITLE || STATUS  ||   LINKS(Shared Posts/Shared Links etc) || COMMENTS' + '\n' + '\n')

        commentsFile = open("Comments.txt", "w", newline='\r\n')
        commentsFile.writelines('username,name,comment' + '\n')

        comments = get_comments(elements[0])
        for comment in comments:
            commentsFile.writelines(comment + '\n')
...
def scrape_data(user_id, scan_list, section, elements_path, save_status, file_names):
    ...
    for i, _ in enumerate(scan_list):
        try:
            ...
            if (save_status == 0) or (save_status == 1) or (save_status == 2):  # Only run this for friends, photos and videos
...
def create_original_link(url):
    if url.find(".php") != -1:
        original_link = facebook_https_prefix + \
            ".facebook.com/" + ((url.split("="))[1])
        ...
        original_link = facebook_https_prefix + \
            ".facebook.com/" + ((url.split("/"))[-1].split("?")[0])
    elif url.find("_tab") != -1:
        original_link = facebook_https_prefix + \
            ".facebook.com/" + (url.split("?")[0]).split("/")[-1]
    else:
        original_link = url
...
def scrap_profile_image(user_id):
    fileName = str(user_id).split('/')[-1] + ".jpg"
    print(fileName)
...
def scrap_facebook_account(facebook_userid, driverInstance):
    global driver
    driver = driverInstance
    
    ids = [facebook_https_prefix + "facebook.com/" + str(facebook_userid)]
...
#        setup_driver()
        scrap_profile(ids)
        os.chdir("../..")
        download_post_images(facebook_userid, driver)
        os.chdir("../../..")
    else:
